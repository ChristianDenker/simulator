...
public class NmeaPosReportUdpOutputBehaviour implements ITickExecution {
	...
	public void execute(Tick tick) {
		...
		double frequency = agent.getScheduler().SchedulerConfiguration().getFreqency_Hz() <= 1.0 ? 1.0 : agent.getScheduler().SchedulerConfiguration().getFreqency_Hz();
